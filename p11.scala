...
for (i <- grid.indices) {
    for(j <- grid(i).indices) {
        if (i < grid.length - 3) {
            val vert = grid(i)(j)*grid(i+1)(j)*grid(i+2)(j)*grid(i+3)(j)
            if (vert>ans) ans = vert
        }
        if (j < grid(i).length - 3) {
            val horiz = grid(i)(j)*grid(i)(j+1)*grid(i)(j+2)*grid(i)(j+3)
            if (horiz>ans) ans = horiz
        }
        if (i < grid.length - 3 && j < grid(i).length - 3) {
            val rightDiag = grid(i)(j)*grid(i+1)(j+1)*grid(i+2)(j+2)*grid(i+3)(j+3)
            if (rightDiag>ans) ans = rightDiag
        }
        if (i > 3 && j < grid(i).length - 3) {
            val leftDiag = grid(i)(j)*grid(i-1)(j+1)*grid(i-2)(j+2)*grid(i-3)(j+3)
            if (leftDiag>ans) ans = leftDiag
        }
    }
}
println(ans)